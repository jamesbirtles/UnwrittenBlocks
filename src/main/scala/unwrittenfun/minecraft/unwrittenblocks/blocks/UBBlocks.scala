...
import unwrittenfun.minecraft.unwrittenblocks.blocks.tileentities.{TEItemScanner, TileEntityDarkInfuser, TileEntityWallTeleporter}
...
//    if (Loader.isModLoaded("ComputerCraft")) {
      itemScanner = new BlockItemScanner(ITEM_SCANNER_ID, ITEM_SCANNER_KEY)
      GameRegistry.registerBlock(itemScanner, ITEM_SCANNER_KEY)
//    }
  }
...
//    if (Loader.isModLoaded("ComputerCraft")) {
      LanguageRegistry.addName(itemScanner, ITEM_SCANNER_NAME)
//    }
  }
...
//    if (Loader.isModLoaded("ComputerCraft")) {
      GameRegistry.registerTileEntity(classOf[TEItemScanner], ITEM_SCANNER_KEY + "TE")
//    }
  }
  ...
  def registerRecipes() {
    ...
    GameRegistry.addRecipe(new ItemStack(darkInfuser, 1), "ooo", "dnd", "nnn", 'o'.asInstanceOf[Character], Block.obsidian, 'd'.asInstanceOf[Character], Item.diamond, 'n'.asInstanceOf[Character], Block.netherBrick)
    GameRegistry.addRecipe(new ItemStack(itemScanner, 1), "ili", "ghg", "ili", 'i'.asInstanceOf[Character], Item.ingotIron, 'l'.asInstanceOf[Character], Block.redstoneLampIdle, 'g'.asInstanceOf[Character], Block.glass, 'h'.asInstanceOf[Character], Block.hopperBlock)
