package unwrittenfun.minecraft.unwrittenblocks

/**
 * Mod: UnwrittenBlocks
 * Author: UnwrittenFun
 * License: Minecraft Mod Public License (Version 1.0.1)
 */
package object blocks {
  var PLEATHER_PLANT_ID = 0
  final val PLEATHER_PLANT_DEFAULT_ID = 3700
  final val PLEATHER_PLANT_KEY = "pleatherPlant"
  final val PLEATHER_PLANT_NAME = "Pleather Plant"

  var WALL_TELEPORTER_ID = 0
  final val WALL_TELEPORTER_DEFAULT_ID = 3701
  final val WALL_TELEPORTER_KEY = "wallTeleporter"
  final val WALL_TELEPORTER_NAME = "Wall Teleporter"

  var DARK_INFUSER_ID = 0
  final val DARK_INFUSER_DEFAULT_ID = 3702
  final val DARK_INFUSER_KEY = "darkInfuser"
  final val DARK_INFUSER_NAME = "Dark Infuser"

  var ITEM_SCANNER_ID = 0
  final val ITEM_SCANNER_DEFAULT_ID = 3703
  final val ITEM_SCANNER_KEY = "itemScanner"
  final val ITEM_SCANNER_NAME = "Item Scanner"
}
