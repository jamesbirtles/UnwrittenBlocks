...
public class RefulgentBrickDyeRecipe implements IRecipe {
  @Override
  public boolean matches(InventoryCrafting crafting, World world) {
    ...
    for (int slot = 0; slot < crafting.getSizeInventory(); slot++) {
      ...
      if (slot == 4) {
        ...
        if (stack == null || stack.getItem() != (new ItemStack(BlockRegister.refulgentBrick)).getItem()) {
          return false;
        }
      }
    }
  ...
  public ItemStack getCraftingResult(InventoryCrafting crafting) {
    ...
    for (int i = 0; i < dyeOreIds.length; i++) {
      ...
      if(dyeStrings.contains(dyeOreName)) {
        return new ItemStack(BlockRegister.refulgentBrick, 8, dyeStrings.indexOf(dyeOreName));
      }
    }
    return null;
  }
